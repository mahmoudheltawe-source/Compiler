...
let rec macro_expand_qq sexpr =
  let quote sexpr = ScmPair (ScmSymbol "quote", ScmPair (sexpr, ScmNil)) in
  let make_app proc args =
    ScmPair (proc, scheme_list_of_ocaml_list args ScmNil) in
  let cons a b = make_app (ScmSymbol "cons") [a; b] in
  let append a b = make_app (ScmSymbol "append") [a; b] in
  match sexpr with
  | ScmPair (ScmSymbol "unquote", ScmPair (e, ScmNil)) -> e
  | ScmPair (ScmSymbol "unquote-splicing", ScmPair (_, ScmNil)) ->
     raise (X_syntax "unquote-splicing: illegal here")
  | ScmNil
  | ScmVoid
  | ScmBoolean _ | ScmChar _ | ScmString _ | ScmNumber _ | ScmSymbol _ ->
     quote sexpr
  | ScmVector sexprs ->
     let lst = scheme_list_of_ocaml_list sexprs ScmNil in
     make_app (ScmSymbol "list->vector") [macro_expand_qq lst]
  | ScmPair (car, cdr) ->
     (match car with
      | ScmPair (ScmSymbol "unquote-splicing", ScmPair (e, ScmNil)) ->
         append e (macro_expand_qq cdr)
      | _ ->
  ...
  (* Expand internal `define` forms that appear at the beginning of a lambda body.
     We rewrite:
       (lambda (args...) (define x e) (define y f) body...)
     into:
       (lambda (args...) (letrec ((x e) (y f)) body...))
  *)
  let expand_internal_defines exprs =
    let normalize_define = function
      | ScmPair (ScmSymbol "define",
                 ScmPair (ScmPair (ScmSymbol var, params), body)) ->
         (* (define (f args) body) -> (define f (lambda (args) body)) *)
         Some (var, ScmPair (ScmSymbol "lambda", ScmPair (params, body)))
      | ScmPair (ScmSymbol "define",
                 ScmPair (ScmSymbol var, ScmPair (value, ScmNil))) ->
         Some (var, value)
      | _ -> None
    in
    let rec split defs = function
      | [] -> (List.rev defs, [])
      | sexpr :: rest ->
         (match normalize_define sexpr with
          | Some (var, value) -> split ((var, value) :: defs) rest
          | None -> (List.rev defs, sexpr :: rest))
    in
    let (defs, rest) = split [] exprs in
    if defs = [] then
      ScmPair (ScmSymbol "begin",
               scheme_list_of_ocaml_list exprs ScmNil)
    else
      let rest =
        if rest = [] then [ScmVoid]
        else
          (if ormap
                (fun e ->
                  match normalize_define e with
                  | Some _ -> true
                  | None -> false)
                rest
           then raise (X_syntax "Internal definitions must precede expressions")
           else rest)
      in
      let ribs =
      ...
      ScmPair (ScmSymbol "letrec",
               ScmPair (ribs,
                        scheme_list_of_ocaml_list rest ScmNil));;
...
let rec macro_expand_cond_ribs ribs =
  let beginify = function
    | ScmNil -> ScmVoid
    | ScmPair (e, ScmNil) -> e
    | exprs -> ScmPair (ScmSymbol "begin", exprs) in
  let make_lambda0 body =
  ...
  | ScmPair (rib, ribs') ->
     (match rib with
      | ScmPair (ScmSymbol "else", exprs) ->
         (match ribs' with
          | ScmNil -> beginify exprs
          | _ -> raise (X_syntax "cond: else must be last"))
      | ScmPair (test,
                 ScmPair (ScmSymbol "=>", ScmPair (expr, ScmNil))) ->
         let rest_expanded = macro_expand_cond_ribs ribs' in
         let value = ScmSymbol "value" in
         let f = ScmSymbol "f" in
         let rest = ScmSymbol "rest" in
         let ribs =
           scheme_list_of_ocaml_list
             [ScmPair (value, ScmPair (test, ScmNil));
              ScmPair (f, ScmPair (make_lambda0 expr, ScmNil));
              ScmPair (rest, ScmPair (make_lambda0 rest_expanded, ScmNil))]
             ScmNil in
         let f_call = ScmPair (f, ScmNil) in
         let apply_f_to_value = ScmPair (f_call, ScmPair (value, ScmNil)) in
         let call_rest = ScmPair (rest, ScmNil) in
         ScmPair
           (ScmSymbol "let",
            ScmPair
              (ribs,
               ScmPair
                 (ScmPair
                    (ScmSymbol "if",
                     ScmPair (value,
                              ScmPair (apply_f_to_value,
                                       ScmPair (call_rest, ScmNil)))),
                  ScmNil)))
      | ScmPair (test, exprs) ->
         let rest_expanded = macro_expand_cond_ribs ribs' in
         let dit =
      ...
      | _ -> raise (X_syntax "cond: malformed rib"))
  | _ -> raise (X_syntax "cond: ribs should be a proper list");;
   ...
   let rec tag_parse sexpr =
     match sexpr with
     | ScmVoid | ScmNil | ScmVector _ | ScmBoolean _ | ScmChar _ | ScmString _ | ScmNumber _ ->
       ScmConst sexpr
     ...
     | ScmPair (ScmSymbol "lambda", ScmPair (params, exprs)) ->
        let exprs =
          (match (scheme_list_to_ocaml exprs) with
           | exprs, ScmNil -> exprs
           | _ -> raise (X_syntax "malformed lambda body: not a proper list")) in
        let expr = tag_parse (expand_internal_defines exprs) in
 ...
 module Semantic_Analysis : SEMANTIC_ANALYSIS = struct
   let copy_list = List.map (fun x -> x);;
   ...
   let annotate_lexical_address =
     let rec run expr params env =
       ...
       | ScmLambda (params', Simple, expr) ->
         let env' = (copy_list params) :: env in
         ScmLambda' (params', Simple, run expr params' env')
       | ScmLambda (params', Opt opt, expr) ->
         let env' = (copy_list params) :: env in
         let params_for_body = params' @ [opt] in
         ScmLambda' (params', Opt opt, run expr params_for_body env')
       | ScmApplic (proc, args) ->
...
let test2 = Code_Generation.compile_scheme_string "testing/goo";;
