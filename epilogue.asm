...
.fill_rib:
        cmp     rdx, rcx
        je      .rib_done
        mov     rax, qword [rbp + 8*4 + 8*rdx]
        mov     qword [r11 + 8*rdx], rax
        inc     rdx
        jmp     .fill_rib
...
.copy_env:
        cmp     rdx, r9
        je      .done
        mov     rax, qword [rsi + 8*rdx]
        mov     qword [r10 + 8*rdx + 8], rax
        inc     rdx
        jmp     .copy_env
...
.L_apply_copy_loop:
        test rcx, rcx
        je .L_apply_copy_done
        mov rax, SOB_PAIR_CAR(rsi)
        stosq
        mov rsi, SOB_PAIR_CDR(rsi)
        dec rcx
        jmp .L_apply_copy_loop
.L_apply_copy_done:

        mov rsp, r13

        cmp byte [r12], T_closure
